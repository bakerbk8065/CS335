...
public class UWECImage {
    ...
    private class ImagePanel extends JPanel {
        ...
        public ImagePanel(UWECImage im) {
            this.im = im;  // Reference to the image - not a copy
                           // So if you call repaint() on this class
                           // it will redraw the updated pixels
            ...
            // Set a fake size for now to get it up -
            // we will never see this show up in these dimensions
            ...
            frame.setSize(im.getWidth() + fInsets.left +
                fInsets.right, im.getHeight() + fInsets.top + fInsets.bottom);
        }
    ...
    // To mutate the existing window to fit the image's
    // current dimensions and pixels
    ...
    public void rotate90() {
        int intH=this.getHeight();
        int intW=this.getWidth();
        UWECImage rotated = new UWECImage(intH,intW);
        for (int i=0; i<intW; i++) {
            for (int j=0; j<intH; j++) {
                rotated.setRGB(intH-j-1, i, this.getRed(i,j), this.getGreen(i,j), this.getBlue(i,j));
            }
        }
        this.im = rotated.im;
    }
    ...
    public void rotate270() {
        ...
        for (int i=0; i<intW; i++) {
            for (int j=0; j<intH; j++) {
                rotated.setRGB(j, intW-i-1, this.getRed(i,j), this.getGreen(i,j), this.getBlue(i,j));
            }
        }
        this.im = rotated.im;
    }
